...
contract MediVault is AccessControl {
    ...
    modifier onlyAdmin() {
        require(
            ...
            "Only superAdmin has permission to do that action"
        );
        _;
    }
    ...
    function addhospital(
        ...
        require(
            !hasRole(HOSPITAL_ROLE, _address),
            "This hospital is really exisit!! "
        );
    ...
    function removeitem_hos(address _hosAddress) public {
        ...
        for (uint i = 0; i < Hospitals_keys.length; i++) {
            if (
                keccak256(abi.encodePacked(Hospitals_keys[i])) ==
                keccak256(abi.encodePacked(_hosAddress))
            ) {
    ...
    function addDoctor(
        ...
        uint256 _age,
        string memory _place,
        string memory _Medical_specialty,
        address _docAddress
    ) public onlyAdmin returns (bool success) {
        require(
            !hasRole(DOCTOR_ROLE, _docAddress),
            "This account already a doctor"
        );
        require(!hasRole(ADMIN_ROLE, _docAddress), "This Account is Admin!! ");
        require(
            !hasRole(HOSPITAL_ROLE, _docAddress),
            "This Account is Hospital!! "
        );
        require(
            !hasRole(PATIENT_ROLE, _docAddress),
            "This Account is Patient!! "
        );
        require(
            hasRole(HOSPITAL_ROLE, _hospital_addr),
            "This hospital is really exisit!! "
        );
        _setupRole(DOCTOR_ROLE, _docAddress);
        Doctor_index = Doctor_index + 1;
        doctors[_docAddress] = doctor(
            Doctor_index,
            _name,
            _hospital_name,
            _hospital_addr,
    ...
    function removeitem_doc(address _docAddress) public {
        ...
        for (uint i = 0; i < doctors_keys.length; i++) {
            if (
                keccak256(abi.encodePacked(doctors_keys[i])) ==
                keccak256(abi.encodePacked(_docAddress))
            ) {
    ...
    function get_hospita_by_address(
        address _addr
    ...
    ) public onlyHospital returns (bool success) {
        require(
            !hasRole(PATIENT_ROLE, _PatientAddress),
            "This account already a patient"
        );
        require(
            !hasRole(ADMIN_ROLE, _PatientAddress),
            "This Account is Admin!! "
        );
        require(
            !hasRole(DOCTOR_ROLE, _PatientAddress),
            "This Account is Doctor!! "
        );
        require(
    ...
    function checkRequestExists(
        address _from_doctor_addr,
        address _to_patients_addr
    ) public view returns (bool) {
        for (uint i = 0; i < requests.length; i++) {
            if (
                ((requests[i].from_doctor_addr) == (_from_doctor_addr)) &&
                ((requests[i].to_patients_addr) == (_to_patients_addr))
            ) {
                return true;
            }
        }
    ...
    function send_request_Access(
        address _patient,
        address _doctor
    ) public returns (bool success) {
        require(
            hasRole(PATIENT_ROLE, _patient),
            "The person you are requesting access to is not a Patient"
        );
        require(
            !checkRequestExists(_doctor, _patient),
    ...
    function addRecord(
        ...
        string memory _rec_name,
        string memory _Created_at,
        address _patientAddr,
        address _doctor_addr,
        string memory _hex_ipfs,
        string memory _notes
    ) public onlydoctors returns (bool) {
        require(
            hasRole(PATIENT_ROLE, _patientAddr),
            "This patients is not exisit"
        );
        require(
            isAuth[_patientAddr][msg.sender],
            "No permission to add Records"
        );
        require(_doctor_addr == msg.sender, "No permission to this Doctor");
        Record_index = Record_index + 1;
        patients[_patientAddr].Records.push(
            record(
                Record_index,
                _category,
                _patient_name,
    ...
    function get_patient_by_address(
        address _addr
    ) public view returns (patient memory) {
        return (patients[_addr]);
    }

    function See_Record_for_Patient() public view returns (record[] memory) {
        require(
            patients[msg.sender].Records.length > 0,
    ...
    function DeleteRequest(address doc, address pat) public {
        ...
        for (uint i = 0; i < requests.length; i++) {
            if (
                (keccak256(abi.encodePacked(requests[i].from_doctor_addr)) ==
                    keccak256(abi.encodePacked(doc))) &&
                (keccak256(abi.encodePacked(requests[i].to_patients_addr)) ==
                    keccak256(abi.encodePacked(pat)))
            ) {
                delete requests[i];
